...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            no_users = len(
                User.objects.filter(timestamp__gte=last, timestamp__lt=now)
            )
            no_question = len(
                Question.objects.filter(pub_date__gte=last, pub_date__lt=now)
            )
            no_answer = len(
                Answer.objects.filter(pub_date__gte=last, pub_date__lt=now)
            )
            no_question_comment = len(
                QuestionComment.objects.filter(
                    pub_date__gte=last, pub_date__lt=now
                )
            )
            no_answer_comment = len(
            ...
            sendemail(
                "samajisegiri@gmail.com",
                no_users,
                no_question,
                no_answer,
                no_question_comment,
                no_answer_comment,
                last.strftime("%A %w, %b %Y."),
                now.strftime("%A %w, %b %Y."),
            )
